...
export default function TaskDetailModal({
  ...
  const STATUS_OPTIONS = [
    ...
    { value: 'in-progress', label: t('tasks.statuses.in-progress') },
    { value: 'review', label: t('tasks.statuses.review') },
    { value: 'done', label: t('tasks.statuses.done') },
    { value: 'deferred', label: t('tasks.statuses.deferred') },
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorPayload.message ?? t('messages.operationFailed'));
      }
      ...
      alert(error instanceof Error ? error.message : t('messages.operationFailed'));
    } finally {
      setIsSaving(false);
    }
  };
          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <button
                ...
                title={t('task.copyId')}
              >
                <span>{t('task.taskPrefix')} {task.id}</span>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-4 md:p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('task.status')}</label>
              <select
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('task.priority')}</label>
              <div className={cn('px-3 py-2 rounded-md text-sm font-medium capitalize', getPriorityBadgeClass(task.priority))}>
                {task.priority ?? t('task.noPriority')}
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('task.dependencies')}</label>
              {Array.isArray(task.dependencies) && task.dependencies.length > 0 ? (
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('task.noDependencies')}</span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {isEditMode ? (
              ...
              <p className="whitespace-pre-wrap text-gray-700 dark:text-gray-300">{task.description || t('task.noDescription')}</p>
            )}
          </div>
          ...
          {task.details && (
            <div className="rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('task.implementation')}</span>
          ...
          {task.testStrategy && (
            <div className="rounded-lg border border-gray-200 dark:border-gray-700">
              <button
                ...
                className="flex w-full items-center justify-between p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-800"
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('task.testStrategy')}</span>
