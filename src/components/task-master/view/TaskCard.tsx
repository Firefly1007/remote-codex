...
function getStatusStyle(status: string | undefined, t: (key: string) => string): TaskStatusStyle {
  if (status === 'done') {
    return {
      icon: CheckCircle,
      statusText: t('task.done'),
...
function renderPriorityIcon(priority: string | undefined, t: (key: string) => string) {
  if (priority === 'high') {
    return (
      <Tooltip content={t('task.highPriority')}>
  ...
  if (priority === 'medium') {
    return (
      <Tooltip content={t('task.mediumPriority')}>
  ...
  if (priority === 'low') {
    return (
      <Tooltip content={t('task.lowPriority')}>
  ...
  return (
    <Tooltip content={t('task.noPriority')}>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <Tooltip content={`${t('task.taskId')}: ${task.id}`}>
          ...
          {showParent && task.parentId && (
            <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{t('task.taskPrefix')} {task.parentId}</span>
          )}
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          {Array.isArray(task.dependencies) && task.dependencies.length > 0 && (
            <Tooltip content={`${t('task.dependsOn')}: ${task.dependencies.map((dependency) => `${t('task.taskPrefix')} ${dependency}`).join(', ')}`}>
              <div className="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400">
                <ArrowRight className="h-3 w-3" />
                <span>{t('task.dependsOn')}: {task.dependencies.join(', ')}</span>
              </div>
            </Tooltip>
          )}
        </div>

        <Tooltip content={`${t('task.status')}: ${statusStyle.statusText}`}>
      ...
      {progress.total > 0 && (
        <div className="ml-3">
          <div className="mb-1 flex items-center gap-2">
            <span className="text-xs text-gray-500 dark:text-gray-400">{t('task.progress')}:</span>
            <div className="h-1.5 flex-1 rounded-full bg-gray-200 dark:bg-gray-700" title={t('task.subtasksCompleted', { completed: progress.completed, total: progress.total })}>
              <div
