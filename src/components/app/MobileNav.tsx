...
export default function MobileNav({ activeTab, setActiveTab, isInputFocused }: MobileNavProps) {
  ...
  const baseCoreItems: CoreNavItem[] = [
    ...
    { id: 'shell', icon: Terminal, label: t('nav.shell') },
    { id: 'files', icon: Folder, label: t('nav.files') },
    { id: 'git', icon: GitBranch, label: t('nav.git') },
  ];
  const coreItems: CoreNavItem[] = shouldShowTasksTab
    ? [...baseCoreItems, { id: 'tasks', icon: ClipboardCheck, label: t('nav.tasks') }]
    : baseCoreItems;
