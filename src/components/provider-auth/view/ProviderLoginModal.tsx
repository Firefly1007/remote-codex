...
const getProviderTitle = (provider: CliProvider, t: (key: string) => string) => {
  if (provider === 'claude') return t('providerAuth.claudeLogin');
  if (provider === 'cursor') return t('providerAuth.cursorLogin');
  if (provider === 'codex') return t('providerAuth.codexLogin');
  if (provider === 'kimi') return t('providerAuth.kimiLogin');
  return t('providerAuth.geminiConfig');
};
...
export default function ProviderLoginModal({
  ...
  const title = getProviderTitle(provider, t);
      ...
      <div className="flex h-3/4 w-full max-w-4xl flex-col rounded-lg bg-white shadow-xl dark:bg-gray-800 max-md:m-0 max-md:h-full max-md:max-w-none max-md:rounded-none md:m-4 md:h-3/4 md:max-w-4xl md:rounded-lg">
        <div className="flex items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
          ...
          <button
            ...
            aria-label={t('providerAuth.closeModal')}
          >
            <X className="h-6 w-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-hidden">
          {provider === 'gemini' ? (
            <div className="flex h-full flex-col items-center justify-center bg-gray-50 p-8 text-center dark:bg-gray-900/50">
              ...
              <h4 className="mb-3 text-xl font-medium text-gray-900 dark:text-white">{t('providerAuth.setupGemini')}</h4>

              <p className="mb-8 max-w-md text-gray-600 dark:text-gray-400">
                {t('providerAuth.geminiDesc')}
              </p>
              ...
              <div className="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 text-left shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <ol className="space-y-4">
                  <li className="flex gap-4">
                    ...
                    <div>
                      <p className="mb-1 text-sm font-medium text-gray-900 dark:text-white">{t('providerAuth.getApiKey')}</p>
                      <a
                      ...
                      >
                        {t('providerAuth.googleAiStudio')} <ExternalLink className="h-3 w-3" />
                      </a>
                    </div>
                  </li>
                  <li className="flex gap-4">
                    ...
                    <div>
                      <p className="mb-1 text-sm font-medium text-gray-900 dark:text-white">{t('providerAuth.runConfig')}</p>
                      <p className="mb-2 text-sm text-gray-600 dark:text-gray-400">{t('providerAuth.openTerminal')}</p>
