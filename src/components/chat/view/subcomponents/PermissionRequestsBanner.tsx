  ...
  return (
    <div className="mb-3 space-y-2">
      {pendingPermissionRequests.map((request) => {
        ...
        const rememberLabel = alreadyAllowed ? t('permission.allowSaved') : t('permission.allowRemember');
        ...
        return (
          ...
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-sm font-semibold text-amber-900 dark:text-amber-100">{t('permission.required')}</div>
                <div className="text-xs text-amber-800 dark:text-amber-200">
                  {t('permission.tool')}: <span className="font-mono">{request.toolName}</span>
                </div>
              </div>
              {permissionEntry && (
                <div className="text-xs text-amber-700 dark:text-amber-300">
                  {t('permission.allowRule')}: <span className="font-mono">{permissionEntry}</span>
                </div>
              )}
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => handlePermissionDecision(request.requestId, { allow: false, message: t('permission.userDenied') })}
