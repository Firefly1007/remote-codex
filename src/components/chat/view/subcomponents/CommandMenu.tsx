...
const namespaceLabelKeys: Record<string, string> = {
  frequent: 'commandMenu.frequentlyUsed',
  builtin: 'commandMenu.builtinCommands',
  project: 'commandMenu.projectCommands',
  user: 'commandMenu.userCommands',
  other: 'commandMenu.otherCommands',
};
...
export default function CommandMenu({
  ...
  return (
    <div
      ref={menuRef}
      role="listbox"
      aria-label={t('commandMenu.availableCommands')}
        ...
        <div key={namespace} className="command-group">
          {orderedNamespaces.length > 1 && (
            <div className="px-3 pb-1 pt-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
              {t(namespaceLabelKeys[namespace] || '') || namespace}
            </div>
          )}
