...
export default function Onboarding({ onComplete }: OnboardingProps) {
    ...
    try {
      ...
      setProviderStatuses((previous) => ({
        ...
        [provider]: {
          ...
          email: null,
          loading: false,
          error: caughtError instanceof Error ? caughtError.message : t('messages.unknownError'),
        },
      }));
    }
  }, [t]);
  ...
  const handleNextStep = async () => {
    ...
    if (!gitName.trim() || !gitEmail.trim()) {
      setErrorMessage(t('onboarding.bothRequired'));
      return;
    }

    if (!gitEmailPattern.test(gitEmail)) {
      setErrorMessage(t('onboarding.invalidEmail'));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.failedGitConfig'));
        throw new Error(message);
      }

      setCurrentStep((previous) => previous + 1);
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.failedGitConfig'));
    } finally {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const message = await readErrorMessageFromResponse(response, t('onboarding.failedOnboarding'));
        throw new Error(message);
      }

      await onComplete?.();
    } catch (caughtError) {
      setErrorMessage(caughtError instanceof Error ? caughtError.message : t('onboarding.failedOnboarding'));
    } finally {
      setIsSubmitting(false);
    }
  };
