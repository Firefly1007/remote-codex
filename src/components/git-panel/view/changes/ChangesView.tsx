  ...
  const requestFileAction = useCallback(
    (filePath: string, status: FileStatusCode) => {
      if (status === 'U') {
        onRequestConfirmation({
          type: 'delete',
          message: t('gitExtra.deleteConfirm', { file: filePath }),
          onConfirm: async () => {
      ...
      onRequestConfirmation({
        type: 'discard',
        message: t('gitExtra.discardConfirm', { file: filePath }),
        onConfirm: async () => {
      ...
      <div className={`flex-1 overflow-y-auto ${isMobile ? 'pb-mobile-nav' : ''}`}>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-center">
            ...
            <h3 className="mb-2 text-lg font-medium text-foreground">{t('gitExtra.noCommitsYet')}</h3>
            <p className="mb-6 max-w-md text-sm text-muted-foreground">
              {t('gitExtra.noCommitsDesc')}
            </p>
            <button
            ...
            >
              {isCreatingInitialCommit ? (
                <>
                  ...
                  <span>{t('gitExtra.creatingInitial')}</span>
                </>
              ) : (
                <>
                  <GitCommit className="h-4 w-4" />
                  <span>{t('gitExtra.createInitial')}</span>
                </>
              )}
            </button>
          </div>
          ...
          <div className="flex h-32 flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="text-sm">{t('gitExtra.noChanges')}</p>
          </div>
        ) : (
