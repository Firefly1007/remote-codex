...
export default function CommitComposer({
  ...
  const requestCommitConfirmation = () => {
    ...
    onRequestConfirmation({
      type: 'commit',
      message: t('git.commitMessage', { count: selectedFileCount, message: trimmedMessage }),
      onConfirm: async () => {
      ...
      {isMobile && isCollapsed ? (
        <div className="border-b border-border/60 px-4 py-2">
          ...
          >
            ...
            <span>{t('git.commit')} {selectedFileCount} 个文件</span>
        ...
        <div className="border-b border-border/60 px-4 py-3">
          {isMobile && (
            <div className="mb-2 flex items-center justify-between">
              <span className="text-sm font-medium text-foreground">{t('git.commitChanges')}</span>
              <button
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-sm text-muted-foreground">
              {t('git.filesSelected', { count: selectedFileCount })}
            </span>
            <button
