...
export default function FileChangeItem({
  ...
  const statusTranslationKeys: Record<FileStatusCode, string> = {
    M: 'git.modified',
    A: 'git.added',
    D: 'git.deleted',
    U: 'git.untracked',
  };
  const statusLabel = t(statusTranslationKeys[status]) || getStatusLabel(status);
      ...
      <div className={`flex items-center transition-colors hover:bg-accent/50 ${isMobile ? 'px-2 py-1.5' : 'px-3 py-2'}`}>
        ...
        <div className="flex min-w-0 flex-1 items-center">
          <button
            ...
            title={isExpanded ? t('git.collapseDiff') : t('git.expandDiff')}
          >
          ...
          <span className="flex items-center gap-1">
            {(status === 'M' || status === 'D' || status === 'U') && (
              <button
                ...
                title={status === 'U' ? t('git.deleteUntracked') : t('git.discardChanges')}
              >
                <Trash2 className="h-3 w-3" />
                {isMobile && <span>{status === 'U' ? t('git.delete') : t('git.discard')}</span>}
              </button>
            )}
        ...
        <div className="flex items-center justify-between border-b border-border p-2">
          ...
          {isMobile && (
            <button
              ...
              title={wrapText ? t('git.switchToScroll') : t('git.switchToWrap')}
            >
              {wrapText ? t('git.scroll') : t('git.wrap')}
            </button>
          )}
        </div>
