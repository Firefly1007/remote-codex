...
export default function GitPanelHeader({
  ...
  const requestPullConfirmation = () => {
    onRequestConfirmation({
      type: 'pull',
      message: t('git.pullMessage', { count: behindCount, remote: remoteName }),
      onConfirm: onPull,
    });
  };
  ...
  const requestPushConfirmation = () => {
    onRequestConfirmation({
      type: 'push',
      message: t('git.pushMessage', { count: aheadCount, remote: remoteName }),
      onConfirm: onPush,
    });
  };
  ...
  const requestPublishConfirmation = () => {
    onRequestConfirmation({
      type: 'publish',
      message: t('git.publishMessage', { branch: currentBranch, remote: remoteName }),
      onConfirm: onPublish,
    });
  };
      ...
      <div className={`flex items-center justify-between border-b border-border/60 ${isMobile ? 'px-3 py-2' : 'px-4 py-3'}`}>
                ...
                <span className="flex items-center gap-1 text-xs">
                  {aheadCount > 0 && (
                    <span
                      ...
                      title={t('gitExtra.ahead', { count: aheadCount })}
                    >
                      {'\u2191'}
                      {aheadCount}
                    </span>
                  ...
                  {behindCount > 0 && (
                    <span
                      ...
                      title={t('gitExtra.behind', { count: behindCount })}
                    >
                      {'\u2193'}
                      {behindCount}
                    </span>
                  ...
                  {remoteStatus.isUpToDate && (
                    <span className="text-muted-foreground" title={t('git.upToDate')}>
                      {'\u2713'}
                    </span>
                  )}
                </span>
        ...
        <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'}`}>
            ...
            <>
              {!remoteStatus.hasUpstream && (
                <button
                  ...
                  title={t('git.publishBranchTitle', { branch: currentBranch, remote: remoteName })}
                >
                ...
                <>
                  {behindCount > 0 && (
                    <button
                      ...
                      title={t('git.pullMessage', { count: behindCount, remote: remoteName })}
                    >
                      <Download className={`h-3 w-3 ${isPulling ? 'animate-pulse' : ''}`} />
                      <span>{isPulling ? t('git.pulling') : `${t('git.pull')} ${behindCount}`}</span>
                    </button>
                  )}
                  ...
                  {aheadCount > 0 && (
                    <button
                      ...
                      title={t('git.pushMessage', { count: aheadCount, remote: remoteName })}
                    >
                      <Upload className={`h-3 w-3 ${isPushing ? 'animate-pulse' : ''}`} />
                      <span>{isPushing ? t('git.pushing') : `${t('git.push')} ${aheadCount}`}</span>
                    </button>
                  )}
          ...
          <button
            ...
            title={t('git.revertLatestCommit')}
          >
            <RotateCcw
