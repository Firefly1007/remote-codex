  ...
  return (
    <div className="flex min-w-0 flex-shrink-0 items-center justify-between border-b border-gray-200 p-4 dark:border-gray-700">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-1">
              <div className="flex min-w-0 flex-1 items-center rounded-md border border-gray-200 bg-gray-50 px-3 py-2 focus-within:border-purple-500 focus-within:ring-2 focus-within:ring-purple-500 dark:border-gray-600 dark:bg-gray-700 dark:focus-within:border-purple-400 dark:focus-within:ring-purple-400">
                <input
                  ...
                  placeholder={t('prd.enterFilename')}
                  maxLength={100}
                />
      ...
      <div className="flex flex-shrink-0 items-center gap-1 md:gap-2">
        <HeaderIconButton
          title={previewMode ? t('prd.switchToEdit') : t('prd.previewMarkdown')}
        ...
        <HeaderIconButton
          title={wordWrap ? t('prd.disableWrap') : t('prd.enableWrap')}
          onClick={onToggleWordWrap}
          icon={<span className="font-mono text-sm font-bold md:text-xs">{t('prd.wrapLabel')}</span>}
          active={wordWrap}
        />

        <HeaderIconButton
          title={t('prd.toggleTheme')}
        ...
        <button
          ...
          title={t('prd.generateTasksTitle')}
        >
          <Sparkles className="h-4 w-4" />
          <span className="hidden md:inline">{t('prd.generateTasks')}</span>
        </button>
        ...
        >
          {saveSuccess ? (
            <>
              ...
              <span className="hidden sm:inline">{t('prd.saved')}</span>
            </>
          ) : (
            <>
              <Save className="h-5 w-5 md:h-4 md:w-4" />
              <span className="hidden sm:inline">{saving ? t('prd.saving') : t('prd.savePrd')}</span>
            </>
          )}
        </button>
        ...
        <button
          ...
          title={isFullscreen ? t('prd.exitFullscreen') : t('prd.fullscreen')}
        >
