...
import { queryKimi, abortKimiSession, getActiveKimiSessions } from './kimi-cli.js';
    ...
    ws.on('message', async (message) => {
        try {
            ...
            if (data.type === 'claude-command') {
                ...
                console.log('📁 Project:', data.options?.projectPath || data.options?.cwd || 'Unknown');
                console.log('🔄 Session:', data.options?.sessionId ? 'Resume' : 'New');
                await queryKimi(data.command, data.options, writer);
                ...
                if (provider === 'cursor') {
                    ...
                    success = await abortKimiSession(data.sessionId);
                } else {
                    // Use Claude Agents SDK
            ...
            for (let i = lines.length - 1; i >= 0; i--) {
                try {
                    ...
                    if (entry.type === 'event_msg' && entry.payload?.type === 'token_count' && entry.payload?.info) {
                        ...
                        // Use last_token_usage (per-turn) for context window fill, not total_token_usage (cumulative)
                        if (tokenInfo.last_token_usage) {
                            totalTokens = tokenInfo.last_token_usage.input_tokens || tokenInfo.last_token_usage.total_tokens || 0;
                        } else if (tokenInfo.total_token_usage) {
                            totalTokens = tokenInfo.total_token_usage.input_tokens || tokenInfo.total_token_usage.total_tokens || 0;
                        }
