...
router.post('/login', async (req, res) => {
  try {
    ...
    let approvedDevice = trustedDevicesDb.getApprovedDevice(user.id, deviceMetadata.deviceId);
    if (!approvedDevice) {
      if (isLocalhostRequest(req)) {
        trustedDevicesDb.approveDevice(user.id, deviceMetadata.deviceId, deviceMetadata);
        approvedDevice = trustedDevicesDb.getApprovedDevice(user.id, deviceMetadata.deviceId);
      } else {
